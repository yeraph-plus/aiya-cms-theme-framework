...
abstract class AYA_Field_Action
{
    ...
    {
        //过滤组件
        if (in_array($field['type'], array('content', 'message', 'success', 'error', 'title_h1', 'title_h2', 'title_h3'))) {
            return self::title_tags($field);
        }
        //回调组件
        if (in_array($field['type'], array('callback'))) {
            $callback_class = self::$class_name . $field['type'];
        ...
        if (!empty($field['id']) && isset($field['id'])) {
            ...
            $new_class = self::$class_name . $field['type'];
    ...
    //内部调用
    public function field_mult($field, $field_action = false)
    {
        //直接提取方法
        if (!empty($field['id']) && isset($field['id'])) {
    ...
    //输出标题
    public function title_tags($field)
    {
        if (empty($field['desc'])) {
            return;
        }
        ...
        switch ($field['type']) {
            case 'title_h1':
                $html .= '<div class="section-title-field">';
                $html .= '<h2>' . $field['desc'] . '</h2>';
                $html .= '</div>';
                break;
            case 'title_h2':
                $html .= '<div class="section-title-field">';
                $html .= '<h3>' . $field['desc'] . '</h3>';
                $html .= '</div>';
                break;
            case 'title_h3':
                $html .= '<div class="section-title-field">';
                $html .= '<h5>' . $field['desc'] . '</h5>';
                $html .= '</div>';
                break;
            case 'content':
    ...
    //报错
    public function out_error($message)
    {
    ...
    //数据检查方法
    public function test_input($value, $type = '')
    {
        switch ($type) {
            case 'name':
                //只包含字母跟空格
                $err = (!preg_match("/^[a-zA-Z ]*$/", $value)) ? __('Only letters and white space allowed', 'AIYA') : '';
                break;
            case 'email':
                //检查邮箱合法性
                $err = (!preg_match("/([\w\-]+\@[\w\-]+\.[\w\-]+)/", $value)) ? __('Invalid email format', 'AIYA') : '';
                break;
            case 'url':
                //检查URL合法性
                $err = (!preg_match("/\b(?:(?:https?|ftp):\/\/|www\.)[-a-z0-9+&@#\/%?=~_|!:,.;]*[-a-z0-9+&@#\/%=~_|]/i", $value)) ? __('Invalid url format', 'AIYA') : '';
                break;
            case 'number':
                //检查是否为数字
                $err = (is_numeric($value)) ? '' : __('Invalid number format', 'AIYA');
                break;
            case 'required':
                //检查是否为空
                $err = empty($value == '') ? __('This is required', 'AIYA') : '';
            default:
                $err = '';
                break;
        }
